...
command -v gdbus >/dev/null 2>&1 || echo "gdbus not found in PATH"
command -v jq    >/dev/null 2>&1 || echo "jq not found in PATH"
command -v pactl >/dev/null 2>&1 || echo "pactl not found in PATH"
...
rc=$?
[[ $rc -ne 0 || -z "$raw" ]] && echo "gdbus call failed (rc=$rc). Is Window Calls enabled?"
...
[[ -z "$win_pid" || "$win_pid" == "null" ]] && echo "couldn't determine focused window's PID"
...
pactl --format=json list sink-inputs >"$sinkfile"
[[ $? -ne 0 ]] && echo "pactl --format=json list sink-inputs failed"
...
if [[ ${#matched_indices[@]} -eq 0 && -n "$win_wmclass" && "$win_wmclass" != "null" ]]; then
    ...
    if [[ "$n_distinct" -gt 1 ]]; then
        rm -f "$sinkfile"
        echo "name fallback matched >1 app — refusing to guess"
    fi
fi

if [[ ${#matched_indices[@]} -eq 0 ]]; then
    rm -f "$sinkfile"
    echo "no active audio stream matched \"$win_title\" (wm_class=$win_wmclass, pid=$win_pid)"
fi
